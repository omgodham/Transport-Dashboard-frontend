...
function Customer() {
    ...
    const [customers, setCustomers] = useState([{ name: 'test', contactInfo: { phoneNo: '30330' } }]);
    const [successSnack, setSuccessSnack] = useState();
    const [errorSnack, setErrorSnack] = useState();
    const [alertMsg, setAlertMsg] = useState('');

    useEffect(() => {
        fetch('http://localhost:3003/customer/getAllCutomers')
        ...
        <div className={classes.root}>
            <Box>
                ...
                <Box className={classes.btnCont}>
                    <Button onClick={() => setOpen(true)} variant="contained" startIcon={<AddCircleOutlineIcon />}>
                        CUSTOMER
                    </Button>
                </Box>
                <Box sx={{ p: 2 }}>
                    {customers?.length ? (
                        customers.map((customer) => (
                            <Box className={classes.customerCont} boxShadow={1}>
                                <Grid container>
