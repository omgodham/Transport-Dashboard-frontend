...
const ComissionCard = ({ isLoading, trips }) => {
    ...
    useEffect(() => {
        if (trips?.length) {
            let tempComission = 0;
            trips.map((trip) => {
                tempComission += trip.commission ? trip.commission : 0;
            });

            tempComission = tempComission.toString();
            var lastThree = tempComission.substring(tempComission.length - 3);
            var otherNumbers = tempComission.substring(0, tempComission.length - 3);
            if (otherNumbers != '') lastThree = ',' + lastThree;
            tempComission = otherNumbers.replace(/\B(?=(\d{2})+(?!\d))/g, ',') + lastThree;

            setTotalComission(tempComission);
        }
    }, [trips]);
                    ...
                    <Grid container direction="column">
                        ...
                        <Grid item>
                            <Grid container alignItems="center">
                                <Grid item>
                                    <Typography className={classes.cardHeading}>₹ {totalComission}</Typography>
                                </Grid>
                                <Grid item>
                        ...
                        <Grid item sx={{ mb: 1.25 }}>
                            <Typography className={classes.subHeading}>Total Commission</Typography>
                        </Grid>
                    </Grid>
                </MainCard>
            )}
